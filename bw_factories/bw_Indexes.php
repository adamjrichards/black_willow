...
abstract class bw_Indexes {
    ...
    function retrieve_the_metadata_from_the_post( $search_for_this, $in_this_post  ) {
            $the_file = file( "rg_assets/rg_posts/$in_this_post" );
    ...
    function sort_the_posts( $this_way ) {
            ...
            $the_posts_sorted_by_filename = array_slice( scandir( "rg_assets/rg_posts" ), 2 );
    ...
    function sort_the_assets_by_title ( $the_asset_type ) {
            ...
            foreach( $the_assets_sorted_by_title as $this_asset ) {
			  if ( \is_file( $this_asset ) ) {
                    if( strpos( $this_asset, MY_PROJECT_PREFIX ) !== FALSE ) {
