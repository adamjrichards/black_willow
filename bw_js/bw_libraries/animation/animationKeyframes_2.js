...
function createSeparateAnimationPropertyStrings( name, duration, timing, delay, iterations, direction, fill ) {
	...
	var animationDuration = "animation-duration: " + duration + "ms;";
	var animationTiming = "animation-timing-function: " + timing + ";";
	var animationDelay = "animation-delay: " + delay + "ms;";
	var animationIterations = "animation-iteration-count: " + iterations + ";";
	var animationDirection = "animation-direction: " + direction + ";";
	var animationFill = "animation-fill-mode: " + fill + ";";
...
function insertAnimationRuleIntoStyleSheet( ruleName, ruleString, styleSheet, index ) {
	...
	allDocumentStyleSheets[ styleSheet ].insertRule( ruleName + " { " + ruleString + " };", index );
} // insertRuleIntoClass()
...
function insertRuleIntoId( element, rule, styleSheet, index ) {
	...
	for ( i = 0; i < allDocumentStyleSheets.length; i++ ) {
		if ( allDocumentStyleSheets[i].title == styleSheet ) {
			allDocumentStyleSheets[i].insertRule( element + ": { " + rule + " };", index );
		} // if
	} // for
} // insertRuleIntoClass()
