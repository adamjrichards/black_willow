...
function checkUsernameAvailability( candidateName, callback ) {
	...
	xmlQuery.open( "POST", "functions/check_username_availability_1.1.1.php", true );
...
function showLogoutConfirmBox() {
	...
	var killSession = window.setTimeout( function() { location = "https://www.deeptext.ca?start_over=true" }, 30000 );
...
function doSetup( panelType ) {
	...
	if ( panelType == "dark_box" ) {
		document.getElementById( "hider_box_1" ).style.display = "block";
		document.getElementById( "hider_box_1" ).style.zIndex = "150";
...
function confirmDropTable( event, context ) {
	console.log( event.type );
	var confirmDrop = confirm( 'Drop this list? This action cannot be undone.' );
	if ( confirmDrop ) {
		context.submit();
	} else {
		event.preventDefault();
		document.getElementById( "drop_table_cancelled" ).style.display = "block";
	} // if else
} // confirmDropTable()

function minimizeHeaderAndUpdateSession( sessionId ) {
	var xmlQuery = new XMLHttpRequest();
	xmlQuery.onreadystatechange = function() {
		if ( xmlQuery.readyState == 4 && xmlQuery.status == 200 ) {
			//console.log( xmlQuery.responseText );
		} // if
	}// onreadystatechange()
	xmlQuery.open( "POST", "functions/minimize_header_and_update_session_1.1.1.php", true );
	xmlQuery.setRequestHeader( "Content-Type", "application/x-www-form-urlencoded" );
	xmlQuery.send( "session_id=" + sessionId );
} // minimizeHeaderAndUpdateSession()

function goBackToLastRelevantPage( tableName, referringPage ) {
	var xmlQuery = new XMLHttpRequest();
	xmlQuery.onreadystatechange = function() {
		if ( xmlQuery.readyState == 4 && xmlQuery.status == 200 ) {
			console.log( xmlQuery.responseText );
	...
	xmlQuery.open( "POST", "functions/go_back_to_last_relevant_page_1.1.1.php", true );
	xmlQuery.setRequestHeader( "Content-Type", "application/x-www-form-urlencoded" );
	xmlQuery.send( "table_name=" + tableName + "&referring_page=" + referringPage );
} // goBackToLastRelevantPage()

function downloadFileFromDatabase( tableName, trackingId, fileId ) {
	var downloadWindow = window.open( "https://www.deeptext.ca/pages/download_file_1.1.1.php?table=" + tableName + "&id=" + trackingId + "&fId=" + fileId, "downloadWindow", "width=1200, height=500, left=100, top=100, menubar=no, toolbar=yes", false );
} // downloadFileFromDatabase()

function trackUploadProgress( uploadIdentifier, progressElement ) {
	console.log( uploadIdentifier );
	var progressBar = document.getElementById( progressElement );
	var track;
	...
	try {
		track = setInterval( function() {
			...
			xmlQuery.onreadystatechange = function() {
				if ( xmlQuery.readyState == 4 && xmlQuery.status == 200 ) {
					if ( xmlQuery.responseText.length > 0 ) console.log( xmlQuery.responseText );
					progressArray = xmlQuery.responseText.split( "&" );
					if ( progressArray[10] == 0 ) {
						clearInterval( track );
						progressBar.value = 100;
					} else {
						if ( ! isNaN( progressArray[7] ) ) {
							progressBar.value = Math.ceil( progressArray[7] / progressArray[8] * 100 );
							console.log( progressBar.value );
						} // if
					} // if else
				} // if
			}// onreadystatechange()
			xmlQuery.open( "POST", "functions/upload_progress_tracker_1.1.1.php", false );
			xmlQuery.setRequestHeader( "Content-Type", "application/x-www-form-urlencoded" );
			xmlQuery.send( "upload_identifier=" + uploadIdentifier );
		}, 400 );
	} catch ( e ) {
...
function getVariablesFromLocation() {
	return location.search.replace( "?", "" ).split( "&" );
} // getVariablesFromLocation()

function toggleSortOrder( context ) {
	var locationArray = getVariablesFromLocation();
	var sortColumn, sortOrder;
	for ( i = 0; i < locationArray.length; i++ ) {
		if ( locationArray[i].indexOf( "sort=" ) != -1 ) {
			sortColumn = locationArray[i].substring( 5 );
			locationArray[i] = "sort=" + context.id;
		} // if
		if ( locationArray[i].indexOf( "order=" ) != -1 ) {
			sortOrder = locationArray[i].substring( 6 );
			if ( sortOrder.indexOf( "asc" ) == 0 ) {
				locationArray[i] = "order=desc";
			} else {
				locationArray[i] = "order=asc"
			} // if else
		} // if
	} // for
	console.log( sortColumn + ", " + sortOrder );
	location = "index.php?" + locationArray.join( "&" );
} // toggleSortOrder()
