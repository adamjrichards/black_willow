...
function setLinkAction( linkNumber, maxLinkNumber, linkColor, backgroundColor, bottom, padding ) {
	var i = 1;
	var linkId, thisLink;
	var delay = setInterval( function() {
		...
		if ( i++ >= maxLinkNumber ) clearInterval( delay );	
	}, 80 );
} // getLinkColor()

function matchSiteLinks( source, maxLink, state ) {
	switch( state ) {
		case "over":
			setLinkAction( source, maxLink, linkColor_over, backgroundColor_over, bottom_over, padding_over );
			break;
		case "down":
			setLinkAction( source, maxLink, linkColor_down, backgroundColor_down, bottom_down, padding_down );
			break;
		case "up":
			setLinkAction( source, maxLink, linkColor_up, backgroundColor_up, bottom_up, padding_up );
			break;
		case "out":
			setLinkAction( source, maxLink, linkColor_out, backgroundColor_out, bottom_out, padding_out );
			break;
		default:
			setLinkAction( source, maxLink, linkColor_out, backgroundColor_out, bottom_out, padding_out );
	} // switch
