...
contentObject.prototype.downloadDocumentFile = function ( filePath, fileName ) {
	...
	this.checkReadiness = function() {
		//console.log( xmlRequest.readyState + ", " + xmlRequest.status + ", " + xmlRequest.statusText );
	...
	if ( this.checkReadiness() ) {
		//console.log( "true" );
		this.xmlDocumentString = String( xmlRequest.responseText );
	} else {
		//console.log( "false" );
		this.downloadDocumentFile( "pages", "ntgq_404_page_1.xml" );
	} // if else
...
contentObject.prototype.buildLinkList = function( convertedStringArray, startingIndex, listType ) {
	...
	while ( convertedStringArray[i].indexOf( "/ntgq_linkList" ) == -1 ) {
		switch ( convertedStringArray[i] ) {
			case "ntgq_ordered_list_title":
				...
				listIsOrdered = true;
				this.htmlString += "#div id=\"ntgq_" + this.baseId + "_container\">#h1 class=\"ntgq_heading_1\" id=\"ntgq_heading_1_" + this.headingCount + "\">" + linkList.title + "#/h1>";
				if ( listType == "simple" ) {
	...
	if ( listType == "simple" ) {
		...
		if ( listIsOrdered ) {
			this.htmlString += "#/ol>#/div>";
		} else {
			this.htmlString += "#/ul>#/div>";
		} // if else
	} else if ( listType == "table" ) {
		var row;
		for( j = 1; j <= this.linkListItemArray.length; j++ ) {
...
contentObject.prototype.buildContentHTML = function() {
	...
	for ( i = 0; i < convertedStringArray.length; i++ ) {
		switch ( convertedStringArray[i] ) {
			...
			case "ntgq_title":
				this.headingCount++;
				this.newStringArray.push( "#h1 class=\"ntgq_content_title\" id=\"ntgq_content_" + this.baseId + "\">" );
			...
			case "ntgq_figure":
				this.figureCount++;
				this.newStringArray.push( "#figure id=\"ntgq_content_figure_"+ this.docName + "_" + this.figureCount + "\">#/figure>" );
				this.newStringArray.push( convertedStringArray[ i + 27 ] );
				this.figureElement = new contentObject.figureObject();
				this.figureElement.name = "ntgq_content_figure_" + this.docName + "_" + this.figureCount;
			...
			case "ntgq_image":
				this.imageCount++;
				this.newStringArray.push( "#img id=\"ntgq_content_image_"+ this.docName + "_" + this.imageCount + "\">" );
				this.newStringArray.push( convertedStringArray[ i + 27 ] );
				this.imageElement = new contentObject.imageObject();
				this.imageElement.name = "ntgq_content_image_" + this.docName + "_" + this.imageCount;
			...
			case "ntgq_media":
				...
				this.mediaElement.name = "ntgq_content_media_" + this.docName + "_" + this.mediaCount;
				this.mediaElement.id = this.mediaElement.name;
				this.mediaElement.className = "ntgq_media ntgq_media_" + this.docName;
				this.mediaElement.title = convertedStringArray[ i + 3 ];
				this.mediaElement.type = convertedStringArray[ i + 7 ];
				this.mediaElement.sourceURL = convertedStringArray[ i + 11 ];
				this.mediaElement.embedCode = convertedStringArray[ i + 15 ].replaceAll( "&lt;", "#" ).replaceAll( "&gt;", ">" );
				this.mediaElement.width = convertedStringArray[ i + 19 ];
				this.mediaElement.height = convertedStringArray[ i + 23 ];
				this.mediaElement.caption = convertedStringArray[ i + 27 ];
				this.mediaElement.citation = convertedStringArray[ i + 31 ];
				this.newStringArray.push(  this.mediaElement.embedCode.replace( "id=\"\"", "id=\"" + this.mediaElement.id + "\"" ) );
				this.newStringArray.push(  "#h3 class=\"ntgq_heading_3\">" + this.mediaElement.caption + "#/h3>" );
				this.newStringArray.push(  "#h5 class=\"ntgq_heading_5\">" + this.mediaElement.citation + "#/h5>" );
				this.mediaArray.push( this.mediaElement );
				break;
			
			case "ntgq_blockquote":
				this.blockquoteCount++;
				this.newStringArray.push( "#blockquote id=\"ntgq_blockquote_" + this.docName + "_" + this.blockquoteCount + "\">#/blockquote>#h6 id=\"ntgq_blockquote_citation_" + this.docName + "_" +  this.blockquoteCount + "\">#a id=\"ntgq_blockquote_citation_link_" + this.docName + "_" + this.blockquoteCount + "\">#/a>#/h6>" );
...
contentObject.prototype.insertFigureElements = function() {
	for ( i = 0; i < this.figureArray.length; i++ ) {
		...
		thisFigure.innerHTML = "#a href=\"" + figureToInsert.href + "\">#img src=\"" + figureToInsert.src + "\" id=\"" + imageToInsertId + "\" class=\"" + imageToInsertClassName + "\" name=\"" + imageToInsertName + "\" title=\"" + figureToInsert.title + "\" width=\"" + figureToInsert.width + "\" height=\"" + figureToInsert.width + "\">#/a>#figcaption>\"" + figureToInsert.caption + "\"#/figcaption>";
	} // for
} // contentObject.prototype.insertLinkElements()
...
contentObject.prototype.insertMediaElements = function() {
	for ( i = 0; i < this.mediaArray.length; i++ ) {
		...
		thisMedia.setAttribute( "src", mediaToInsert.sourceURL );
