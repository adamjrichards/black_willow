...
class BW_Load_Queue {
	...
	public function __construct( $the_handle, $the_family ) {
		...
		if ( ! $the_family->isEmpty() ) {
			...
			foreach ( $the_iterable as $this_kid ) {
				...
				if ( $the_big_sister === "append" ) {
					$the_hierarchy->insert( 0, $the_kid );
				} else {
					$the_big_sisters_index = $the_hierarchy->find( $the_big_sister );
					$this_kids_new_index = $the_big_sisters_index + 1;
					$the_hierarchy->insert( $this_kids_new_index, $the_kid );
				}
     ...
     public function htmlize_it( $the_family = null ) {
		...
		$the_family->reverse();
          foreach ( $the_family->keys() as $this_daddy ) {
			$this->my_codepage .= $GLOBALS[ $this_daddy ]->get_my_node_opener();
			if ( $GLOBALS[ $this_daddy ]->has_children() ) {
                    $this->htmlize_it( $GLOBALS[ $this_daddy ]->get_my_children() );
               }
               $this->my_codepage .= $GLOBALS[ $this_daddy ]->get_my_node_closer();
          }
     }
	...
	public function get_my_project_as_an_array() {
          return $this->my_project->toArray();
     }
